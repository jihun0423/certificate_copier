...
class CertApp:
    ...
    def _setup_window(self):
        ...
        self.root.bind("<Configure>", self._save_geometry_event)
    ...
    def _build_cert_card(self, idx, cert):
        ...
        tk.Button(btns, text=" ✎ 수정 ", bg=SURFACE2, fg=TEXT_MUT,
                  font=FONT_SMALL, bd=0, cursor="hand2", relief="flat", padx=6, pady=4,
                  command=lambda i=idx: self._open_edit_dialog(i)).pack(side="left", padx=2)

        tk.Button(btns, text=" ✕ 삭제 ", bg=DANGER, fg=WHITE,
                  font=FONT_SMALL, bd=0, cursor="hand2", relief="flat", padx=6, pady=4,
                  command=lambda i=idx: self._delete(i)).pack(side="left", padx=2)

        # 카드 헤더(info 영역)만 클릭으로 토글 — btns 영역은 제외
        ...
        bind_toggle(info)
        # 버튼 영역은 토글 바인딩 해제 (버튼 자체 command가 동작하도록)
        btns.unbind("<Button-1>")
    ...
    def _load_geometry(self):
        if os.path.exists(DATA_FILE):
            try:
                ...
                pass
        return "400x560+100+100"

    def _save_geometry_event(self, event=None):
